/**
 * XtremeLeads — Conditional Logic Engine (Sprint 6)
 *
...
(function () {
	...
	function getFieldValues(form, fieldId) {
		var wrap = form.querySelector('.xl-field-wrap[data-field-id="' + fieldId + '"]');
		if (!wrap) {
			return [];
		}
	...
	function evaluateRulesPass(form) {
		...
		rules.forEach(function (rule) {
			...
			var targetWrap = form.querySelector('.xl-field-wrap[data-field-id="' + rule.fieldId + '"]');
			if (!targetWrap) {
				return;
			}
	...
	function evaluateRules(form) {
		...
		if (start && typeof performance !== 'undefined') {
			...
			if (elapsed > 90) {
				console.warn('[XtremeLeads] Conditional logic evaluation took ' + elapsed.toFixed(1) + 'ms (' + passes + ' passes)');
			}
		}
	}
		...
		forms.forEach(function (form) {
			...
			['change', 'input', 'keyup'].forEach(function (evtName) {
				form.addEventListener(evtName, function (e) {
					...
					var changed = e.target.closest('.xl-field-wrap');
					if (!changed) {
						return;
					}
					evaluateRules(form);
