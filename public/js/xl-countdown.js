/**
 * XtremeLeads — Countdown Timer + Scheduled Form Reveal (Sprint 6)
 *
...
(function () {
	...
	function initCountdowns() {
		var containers = document.querySelectorAll('.xl-countdown-wrap');
		containers.forEach(function (wrap) {
			...
			var formWrap    = document.getElementById('xl-form-wrap-' + formId);
			var timerEl     = wrap.querySelector('.xl-countdown-timer');
			...
			var tick = function () {
				...
				if (remaining <= 0) {
					...
					if (formWrap) {
						...
						var closedMsg = document.getElementById('xl-closed-msg-' + formId);
						if (closedMsg) {
				...
				if (timerEl) {
					timerEl.innerHTML =
						'<span class="xl-cd-days">'    + pad(days)    + '<em>' + (formData.i18n.days    || 'd') + '</em></span>' +
						'<span class="xl-cd-hours">'   + pad(hours)   + '<em>' + (formData.i18n.hours   || 'h') + '</em></span>' +
						'<span class="xl-cd-minutes">' + pad(minutes) + '<em>' + (formData.i18n.minutes || 'm') + '</em></span>' +
						'<span class="xl-cd-seconds">' + pad(seconds) + '<em>' + (formData.i18n.seconds || 's') + '</em></span>';
				}
