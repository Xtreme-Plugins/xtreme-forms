...
- **`maybe_upgrade()` is an option-read fast path.** On a fully-migrated site (the common case) the method now returns after a single `get_option( 'xtremeforms_db_version' )` and one `version_compare()` — no schema queries are issued. A per-request static guard prevents `plugins_loaded` firing twice from double-invoking `dbDelta()`.
...
### Removed
- All raw `ALTER TABLE ADD COLUMN` queries from `XF_Activator::create_tables()`, along with the `INFORMATION_SCHEMA.COLUMNS` lookups that gated them. The activator no longer touches schema outside `dbDelta()`.
