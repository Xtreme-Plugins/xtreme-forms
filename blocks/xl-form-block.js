...
(function (blocks, element, blockEditor, components, i18n) {
	...
	var blockData = window.xlBlockData || {};
	var forms     = blockData.forms    || [];

	// Build options for SelectControl.
	var formOptions = [
		{ value: 0, label: __('— Select a Form —', 'xtremeleads') }
	].concat(
	...
	function validateWidth(val) {
		...
		if (pxMatch) {
			...
			if (px < 50)   { return __('Minimum width is 50px.', 'xtremeleads'); }
			if (px > 2000) { return __('Maximum width is 2000px.', 'xtremeleads'); }
			return '';
		}

		if (pctMatch) {
			var pct = parseFloat(pctMatch[1]);
			if (pct < 10)  { return __('Minimum width is 10%.', 'xtremeleads'); }
			if (pct > 100) { return __('Maximum width is 100%.', 'xtremeleads'); }
			return '';
		}

		// Non-numeric / unrecognised format.
		return __('Enter a valid width (e.g. 80%, 400px).', 'xtremeleads');
	}

	blocks.registerBlockType('xtremeleads/form', {
		...
		apiVersion:  3,
		title:       __('XtremeLeads Form', 'xtremeleads'),
		description: __('Embed a lead capture form on any page, post, or FSE template.', 'xtremeleads'),
		icon:        'email-alt',
		category:    'xtremeleads',
		keywords:    [
			__('lead', 'xtremeleads'),
			__('form', 'xtremeleads'),
			__('contact', 'xtremeleads'),
		],
		supports: {
			html:  false,
			align: ['wide', 'full'],
		...
		edit: function (props) {
			...
			var blockProps = useBlockProps({ className: 'xl-block-editor-wrap' });
			...
			var inspector = el(
				...
				el(
					PanelBody,
					{ title: __('Form Settings', 'xtremeleads'), initialOpen: true },
					el(SelectControl, {
						label:    __('Select Form', 'xtremeleads'),
						value:    formId,
						options:  formOptions,
						...
						help: formId === 0
							? __('Choose which lead form to display.', 'xtremeleads')
							: __('Form ID: ', 'xtremeleads') + formId,
					})
				),
				...
				el(
					PanelBody,
					{ title: __('Layout & Styling', 'xtremeleads'), initialOpen: false },

					el(TextControl, {
						label:   __('Width (e.g. 80%, 400px)', 'xtremeleads'),
						value:   width,
						onChange: function (val) {
						...
						help: widthError
							...
							: __('Leave blank or set to 100% for full width.', 'xtremeleads'),
					}),

					el(RadioControl, {
						label:    __('Alignment', 'xtremeleads'),
						selected: alignment,
						options:  [
							{ label: __('Left', 'xtremeleads'),   value: 'left' },
							{ label: __('Center', 'xtremeleads'), value: 'center' },
							{ label: __('Right', 'xtremeleads'),  value: 'right' },
							{ label: __('Full', 'xtremeleads'),   value: 'full' },
						],
						onChange: function (val) {
					...
					el('p', {}, el('strong', {}, __('Background Colour', 'xtremeleads'))),
					el(ColorPicker, {
					...
					backgroundColor
						? el(
							...
							__('Clear background colour', 'xtremeleads')
						  )
						: null
				)
			);
			...
			if (formId === 0) {
				...
				preview = el(
					Placeholder,
					{
						key:   'placeholder',
						icon:  'email-alt',
						label: __('XtremeLeads Form', 'xtremeleads'),
					},
					el('p', { key: 'msg' }, __('Select a form to preview', 'xtremeleads'))
				);
			} else if (!formExists) {
				...
				preview = el(
					...
					el('strong', {}, __('⚠ Form Unavailable', 'xtremeleads')),
					el('p', {}, __('The selected form (ID: ', 'xtremeleads') + formId + __(') no longer exists. Please choose a different form.', 'xtremeleads'))
				);
			} else {
				...
				var formTitle  = selectedForm ? selectedForm.name : (__('Form #', 'xtremeleads') + formId);
				...
				preview = el(
					...
					el('div', {
						...
						el('small', { style: { display: 'block', color: '#6C757D', marginTop: '2px' } }, __('Form ID: ', 'xtremeleads') + formId)
					),
					formFields.length > 0
						? el.apply(null, ['div', { key: 'fields' }].concat(fieldPreviews))
						: el('p', { key: 'no-fields', style: { color: '#6C757D', fontStyle: 'italic', fontSize: '13px' } },
							__('This form has no visible fields configured yet.', 'xtremeleads')
						),
					el('button', {
						key:      'submit-preview',
						disabled: true,
