<?php
/**
 * WordPress Multisite / Network support for XtremeLeads.
 *
 * Responsibilities:
...
class XL_Multisite {
	 ...
	 */
	const NETWORK_SETTINGS_KEY = 'xtremeleads_network_settings';
	 ...
	 */
	const SITE_DISABLED_KEY = 'xtremeleads_site_disabled';
	...
	/**
	 * Check whether XtremeLeads is disabled for the current (or specified) subsite.
	 *
	 ...
	 * Allows a site admin to hide/show XtremeLeads for their subsite.
	 */
	public static function handle_toggle_site_disabled(): void {
		check_admin_referer( 'xl_toggle_site_disabled' );
		...
		wp_safe_redirect(
			add_query_arg(
				array( 'xl_site_toggled' => '1', 'xl_site_disabled' => $new_val ),
	...
	public static function register_network_menu(): void {
		add_menu_page(
			__( 'XtremeLeads Network', 'xtremeleads' ),
		...
		add_submenu_page(
			'xtremeleads-network',
			__( 'Network Dashboard', 'xtremeleads' ),
		...
		add_submenu_page(
			'xtremeleads-network',
			__( 'Global Settings', 'xtremeleads' ),
			__( 'Global Settings', 'xtremeleads' ),
			'manage_network',
	...
	public static function page_network_dashboard(): void {
		...
		require_once XTREMELEADS_PLUGIN_DIR . 'admin/partials/xl-admin-network-dashboard.php';
	}
	...
	public static function page_network_settings(): void {
		...
		require_once XTREMELEADS_PLUGIN_DIR . 'admin/partials/xl-admin-network-settings.php';
	}
	...
	public static function display_network_notices(): void {
		...
		if ( ! empty( $_GET['xl_push_done'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification
			$success = absint( $_GET['xl_push_success'] ?? 0 ); // phpcs:ignore WordPress.Security.NonceVerification
			$failed = absint( $_GET['xl_push_failed'] ?? 0 ); // phpcs:ignore WordPress.Security.NonceVerification
			echo '<div class="notice notice-success is-dismissible"><p>';
			printf(
				/* translators: 1: number of sites updated, 2: number of sites failed */
				esc_html__( 'Settings pushed. %1$d site(s) updated successfully. %2$d site(s) failed.', 'xtremeleads' ),
				(int) $success,
				(int) $failed
			);
			echo '</p>';

			if ( $failed > 0 && ! empty( $_GET['xl_push_failed_ids'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification
				$failed_ids = array_map( 'absint', explode( ',', sanitize_text_field( wp_unslash( $_GET['xl_push_failed_ids'] ) ) ) ); // phpcs:ignore WordPress.Security.NonceVerification
				echo '<p>' . esc_html__( 'Failed site IDs:', 'xtremeleads' ) . ' ';
	...
	public static function handle_push_settings(): void {
		check_admin_referer( 'xl_network_push_settings' );
		...
		$email_template = sanitize_textarea_field( wp_unslash( $_POST['xl_email_template_body'] ?? '' ) );
		$email_header_color = sanitize_hex_color( wp_unslash( $_POST['xl_email_header_color'] ?? '#1A73E8' ) );
		$retention_days = isset( $_POST['xl_retention_days'] ) && '' !== $_POST['xl_retention_days']
			? absint( $_POST['xl_retention_days'] )
			: '';
		$anonymize_ip = isset( $_POST['xl_anonymize_ip'] ) ? '1' : '0';
		...
		foreach ( $blog_ids as $blog_id ) {
			...
			$site_settings = get_option( 'xtremeleads_settings', array() );
			...
			$settings_updated = update_option( 'xtremeleads_settings', $site_settings );
			$template_updated = update_option( 'xtremeleads_email_template', $email_tmpl );

			// Treat as success even if values were already identical (update_option returns false when unchanged).
			$result = ( false !== $settings_updated || $site_settings === get_option( 'xtremeleads_settings' ) )
				&& ( false !== $template_updated || $email_tmpl === get_option( 'xtremeleads_email_template' ) );
		...
		if ( class_exists( 'XL_Audit_Log' ) ) {
			XL_Audit_Log::record(
				XL_Audit_Log::ACTION_GLOBAL_SETTINGS_UPDATED,
				0,
				array(
					'scope' => 'network_push',
					'success' => $success,
		...
		wp_safe_redirect(
			add_query_arg(
				array(
					...
					'xl_push_done' => '1',
					'xl_push_success' => $success,
					'xl_push_failed' => $failed,
					'xl_push_failed_ids' => implode( ',', $failed_ids ),
				),
	...
	public static function get_aggregated_data(): array {
		...
		$plugin_basename = plugin_basename( XTREMELEADS_PLUGIN_FILE );
		$blog_ids = get_sites( array( 'fields' => 'ids', 'number' => 0 ) );
		...
		foreach ( $blog_ids as $blog_id ) {
			...
			foreach ( (array) $site_forms as $row ) {
				$top_forms[] = array(
					'form_name' => $row->form_name ?: __( '(Deleted Form)', 'xtremeleads' ),
					'blog_name' => $blog_name,
