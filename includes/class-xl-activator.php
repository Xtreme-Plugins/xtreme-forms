...
class XL_Activator {
	...
	public static function on_new_blog( $blog ): void {
		...
		if ( is_plugin_active_for_network( plugin_basename( XTREMELEADS_PLUGIN_FILE ) ) ) {
	...
	public static function maybe_upgrade(): void {
		$current_db_version = get_option( 'xtremeleads_db_version', '' );

		// If DB version doesn't match plugin version, run create_tables() to apply any new tables.
		if ( version_compare( $current_db_version, XTREMELEADS_VERSION, '<' ) ) {
			self::create_tables();
			self::set_default_options();
		}
	}
 ...
 KEY email_address (email_address)
		...
		// ── Notes table ──────────────────────────────────────────────────────
		$notes_table = $wpdb->prefix . 'xtremeleads_notes';
 ...
 KEY created_at (created_at)
		...
		// ── Activity table ───────────────────────────────────────────────────
		$activity_table = $wpdb->prefix . 'xtremeleads_activity';
 ...
 KEY tag_id (tag_id)
		...
		// ── Email Log table ────────────────────────────────────────
		$email_log_table = $wpdb->prefix . 'xtremeleads_email_log';
 ...
 KEY sent_at (sent_at)
		...
		// ── Routing Rules table ────────────────────────────────────
		$routing_rules_table = $wpdb->prefix . 'xtremeleads_routing_rules';
 ...
 KEY created_at (created_at)
		...
		// ── Webhooks table ─────────────────────────────────────────
		$webhooks_table = $wpdb->prefix . 'xtremeleads_webhooks';
 ...
 KEY is_active (is_active)
		...
		$webhook_log_table = $wpdb->prefix . 'xtremeleads_webhook_log';
 ...
 KEY delivered_at (delivered_at)
		...
		// ── Spam Log table ─────────────────────────────────────────
		$spam_log_table = $wpdb->prefix . 'xtremeleads_spam_log';
 ...
 KEY created_at (created_at)
		...
		// ── Audit Log table ────────────────────────────────────────
		$audit_log_table = $wpdb->prefix . 'xtremeleads_audit_log';
 ...
 KEY created_at (created_at)
		...
		$forms_table_check = $wpdb->prefix . 'xtremeleads_forms';
	...
	private static function set_default_options(): void {
		...
		if ( false === get_option( 'xtremeleads_settings' ) ) {
			update_option( 'xtremeleads_settings', $defaults );
		}
	}
}
