...
class XL_Leads {
	...
	public static function insert_lead( array $data ): int|false {
		...
		if ( empty( $values ) ) {
			...
			// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.PreparedSQL.NotPrepared, PluginCheck.Security.DirectDB.UnescapedDBParameter, WordPress.DB.PreparedSQLPlaceholders.UnfinishedPrepare, WordPress.DB.PreparedSQLPlaceholders.ReplacementsWrongNumber
			$inserted = $wpdb->query(
				$wpdb->prepare(
	...
	public static function get_statuses(): array {
		return array(
			self::STATUS_NEW => __( 'New', 'xtremeleads' ),
			self::STATUS_CONTACTED => __( 'Contacted', 'xtremeleads' ),
			self::STATUS_QUALIFIED => __( 'Qualified', 'xtremeleads' ),
			self::STATUS_CONVERTED => __( 'Converted', 'xtremeleads' ),
			self::STATUS_LOST => __( 'Lost', 'xtremeleads' ),
			self::STATUS_ARCHIVED => __( 'Archived', 'xtremeleads' ),
		);
	}
