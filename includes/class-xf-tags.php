...
class XF_Tags {
	...
	public static function create_tag( string $name ): int|WP_Error {
		...
		if ( '' === $name ) {
			return new WP_Error( 'empty_name', __( 'Tag name cannot be empty.', 'xtreme-forms' ) );
		}

		$slug = sanitize_title( $name );
		if ( '' === $slug ) {
			return new WP_Error( 'invalid_name', __( 'Tag name is invalid.', 'xtreme-forms' ) );
		}

		global $wpdb;
		$table = $wpdb->prefix . 'xtremeforms_tags';
		...
		if ( $existing ) {
			return new WP_Error(
				...
				sprintf( __( 'A tag named "%s" already exists.', 'xtreme-forms' ), sanitize_text_field( $name ) )
			);
		}
		...
		if ( false === $inserted ) {
			return new WP_Error( 'db_error', __( 'Failed to create tag.', 'xtreme-forms' ) );
		}
	...
	public static function apply_tag_to_lead( int $lead_id, int $tag_id ): bool {
		global $wpdb;
		$table = $wpdb->prefix . 'xtremeforms_lead_tags';
