...
class XL_Email {
	...
	public static function send_new_lead_notification(
		...
		$context = XL_Email_Templates::build_context_from_lead(
			$lead_obj,
			$field_defs,
			$field_values,
			$form_name
		...
		$admin_link = admin_url( 'admin.php?page=xtremeleads&xl_action=view&lead_id=' . $lead_id );

		// Build the email body from the template.
		$email_data = XL_Email_Templates::build_email(
			$context,
			$field_defs,
			$field_values,
			$admin_link,
		...
		$routing_recipients = XL_Routing_Rules::get_matching_recipients( $form_id, $field_values );
		...
		if ( ! empty( $routing_recipients ) ) {
			...
			foreach ( $routing_recipients as $recipient ) {
				...
				$log_id = XL_Email_Log::insert( array(
					'lead_id' => $lead_id,
					'recipient' => $recipient,
					'subject' => $subject,
					'body' => $body,
					'headers' => $headers,
					'trigger_type' => XL_Email_Log::TRIGGER_ROUTING,
					'status' => $sent ? XL_Email_Log::STATUS_SENT : XL_Email_Log::STATUS_FAILED,
					'failure_reason' => $sent ? '' : 'wp_mail() returned false',
				) );

				// Audit log for sent emails.
				if ( $sent && class_exists( 'XL_Audit_Log' ) ) {
					XL_Audit_Log::record(
						XL_Audit_Log::ACTION_EMAIL_SENT,
						$lead_id,
						array(
							'recipient' => $recipient,
							'trigger_type' => XL_Email_Log::TRIGGER_ROUTING,
			...
			if ( empty( $recipients ) ) {
				...
				foreach ( $recipients as $recipient ) {
					...
					$log_id = XL_Email_Log::insert( array(
						...
						'body' => $body,
						'headers' => $headers,
						'trigger_type' => XL_Email_Log::TRIGGER_NOTIFICATION,
						'status' => $sent ? XL_Email_Log::STATUS_SENT : XL_Email_Log::STATUS_FAILED,
						'failure_reason' => $sent ? '' : 'wp_mail() returned false',
					) );
	...
	private static function maybe_send_auto_responder(
		...
		if ( '' === $lead_email || ! is_email( $lead_email ) ) {
			...
			XL_Email_Log::insert( array(
				...
				'body' => '',
				'headers' => '',
				'trigger_type' => XL_Email_Log::TRIGGER_AUTO_RESPONDER,
				'status' => XL_Email_Log::STATUS_SKIPPED,
		...
		if ( '' === $ar_subject ) {
			$ar_subject = sprintf(
				...
				__( 'Thank you for contacting %s', 'xtremeleads' ),
				get_bloginfo( 'name' )
			);
		}

		if ( '' === $ar_body ) {
			$ar_body = __( 'Thank you for your submission. We will be in touch soon.', 'xtremeleads' );
		}

		// Process merge tags in auto-responder content.
		$ar_subject = XL_Email_Templates::process_merge_tags( $ar_subject, $context, false );
		$ar_body = XL_Email_Templates::process_merge_tags( $ar_body, $context, false );
		...
		$dark_color = '#0D1B2A';
		$footer_text = esc_html( XL_Email_Templates::process_merge_tags( $template['footer_text'], $context, false ) );
...
HTML;
	...
	public static function send_test_email(): array {
		...
		if ( ! is_email( $recipient ) ) {
			return array(
				'success' => false,
				'message' => __( 'Could not determine a valid recipient email.', 'xtremeleads' ),
				'log_id' => 0,
			);
		}
		...
		$log_id = XL_Email_Log::insert( array(
			...
			'body' => $body,
			'headers' => $headers,
			'trigger_type' => XL_Email_Log::TRIGGER_TEST,
			'status' => $sent ? XL_Email_Log::STATUS_SENT : XL_Email_Log::STATUS_FAILED,
			'failure_reason' => $sent ? '' : 'wp_mail() returned false',
		) );
		...
		if ( $sent ) {
			return array(
				...
				'message' => sprintf( __( 'Test email sent to %s.', 'xtremeleads' ), esc_html( $recipient ) ),
				'log_id' => (int) $log_id,
			);
		} else {
			return array(
				'success' => false,
				'message' => __( 'Failed to send test email. Please check your WordPress email configuration.', 'xtremeleads' ),
				'log_id' => (int) $log_id,
			);
		}
	}
	...
	public static function resend_from_log( int $log_id ): array {
		$entry = XL_Email_Log::get_log( $log_id );

		if ( ! $entry ) {
			return array(
				'success' => false,
				'message' => __( 'Email log entry not found.', 'xtremeleads' ),
				'new_log_id' => 0,
			);
		}
		...
		if ( ! is_email( $recipient ) ) {
			return array(
				'success' => false,
				'message' => __( 'Invalid recipient address in log entry.', 'xtremeleads' ),
				'new_log_id' => 0,
			);
		}
		...
		$new_log_id = XL_Email_Log::insert( array(
			'lead_id' => (int) $entry->lead_id,
			'recipient' => $recipient,
			'subject' => $entry->subject,
			'body' => $entry->body,
			'headers' => $headers,
			'trigger_type' => XL_Email_Log::TRIGGER_RESEND,
			'status' => $sent ? XL_Email_Log::STATUS_SENT : XL_Email_Log::STATUS_FAILED,
		...
		return array(
			'success' => $sent,
			'message' => $sent
				? __( 'Email resent successfully.', 'xtremeleads' )
				: __( 'Failed to resend email.', 'xtremeleads' ),
