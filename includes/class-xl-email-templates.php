...
/**
 * Class XL_Email_Templates
 */
class XL_Email_Templates {

	/** Option key where the global template settings are stored. */
	const OPTION_KEY = 'xtremeleads_email_template';
	...
	public static function get_defaults(): array {
		return array(
			'logo_url' => '',
			'header_color' => '#1A73E8',
			'subject' => __( '[{{site_name}}] New Lead: {{lead_name}}', 'xtremeleads' ),
			'body_text' => __( 'You have received a new lead submission from your website.', 'xtremeleads' ),
			'footer_text' => __( 'This email was sent by XtremeLeads. Visit {{site_url}} to manage your leads.', 'xtremeleads' ),
		);
	}
	...
	public static function build_email(
		...
		if ( ! empty( $field_defs ) ) {
			foreach ( $field_defs as $field ) {
				...
				if ( '' === $value ) {
					if ( $is_test ) {
						$value = '<em style="color:#6C757D;">[' . esc_html__( 'Sample Value', 'xtremeleads' ) . ']</em>';
					} else {
						$value = '<em style="color:#6C757D;">' . esc_html__( '(not provided)', 'xtremeleads' ) . '</em>';
					}
				}
		...
		if ( '' === $field_rows_html && $is_test ) {
			$field_rows_html = '<tr><td colspan="2" style="padding:16px;color:#6C757D;">' . esc_html__( '[Sample form fields will appear here]', 'xtremeleads' ) . '</td></tr>';
		} elseif ( '' === $field_rows_html ) {
			$field_rows_html = '<tr><td colspan="2" style="padding:16px;color:#6C757D;">' . esc_html__( 'No field data submitted.', 'xtremeleads' ) . '</td></tr>';
		}

		// Source URL and admin link rows.
		$source_url = esc_url( $context['source_url'] ?? '' );
		$admin_link = esc_url( $admin_link ?: ( $context['admin_link'] ?? '' ) );
		$label_source = esc_html__( 'Source Page', 'xtremeleads' );
		$label_view = esc_html__( 'View Lead in Admin', 'xtremeleads' );
		$label_btn = esc_html__( 'View Lead', 'xtremeleads' );
...
HTML;
	...
	public static function build_context_from_lead(
		...
		$admin_link = admin_url( 'admin.php?page=xtremeleads&xl_action=view&lead_id=' . absint( $lead->id ) );
