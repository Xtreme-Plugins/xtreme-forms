...
class XL_Ajax {
	...
	private function require_admin_auth( string $nonce_action, string $capability = 'manage_options' ): void {
		...
		if ( ! is_user_logged_in() ) {
			...
			wp_send_json_error(
				array(
					...
					'message' => __( 'Authentication required.', 'xtremeleads' ),
				),
				401
			);
		}
		...
		if ( ! wp_verify_nonce( $nonce, $nonce_action ) ) {
			...
			wp_send_json_error(
				array(
					'code' => 'invalid_nonce',
					'message' => __( 'Security check failed. Please refresh the page and try again.', 'xtremeleads' ),
				),
				403
			);
		}
		...
		if ( ! current_user_can( $capability ) ) {
			...
			wp_send_json_error(
				array(
					...
					'message' => __( 'You do not have permission to perform this action.', 'xtremeleads' ),
				),
				403
			);
		}
	...
	private function check_admin_ajax(): void {
		$this->require_admin_auth( 'xl_admin_nonce' );
	}
	...
	public function handle_form_submit(): void {
		// Verify nonce.
		$form_id = isset( $_POST['xl_form_id'] ) ? absint( $_POST['xl_form_id'] ) : 0;

		if ( ! $form_id ) {
			wp_send_json_error( array( 'message' => __( 'Invalid form.', 'xtremeleads' ) ), 400 );
		}

		$nonce = isset( $_POST['xl_nonce'] ) ? sanitize_text_field( wp_unslash( $_POST['xl_nonce'] ) ) : '';
		if ( ! wp_verify_nonce( $nonce, 'xl_form_submit_' . $form_id ) ) {
			wp_send_json_error( array( 'message' => __( 'Security check failed. Please refresh the page and try again.', 'xtremeleads' ) ), 403 );
		}
		...
		$ip_key = 'xl_rl_' . md5( $visitor_ip_rl . '_' . $form_id );
		$rl_count = (int) get_transient( $ip_key );
		$rl_limit = apply_filters( 'xtremeleads_rate_limit_per_form', 10 );

		if ( $rl_count >= $rl_limit ) {
			wp_send_json_error(
				array( 'message' => __( 'Too many submissions. Please wait a moment before trying again.', 'xtremeleads' ) ),
				429
			);
		}
		...
		$source_url_early = isset( $_SERVER['HTTP_REFERER'] )
			...
			: ( isset( $_POST['xl_source_url'] ) ? esc_url_raw( wp_unslash( $_POST['xl_source_url'] ) ) : '' );
		...
		// Honeypot check: the xl_website_url field must be blank.
		// Fail silently — return a success-like response to avoid revealing
		// spam detection to bots. Log in spam_log table.
		$honeypot_value = isset( $_POST['xl_website_url'] )
			? sanitize_text_field( wp_unslash( $_POST['xl_website_url'] ) )
			: '';
		if ( '' !== $honeypot_value ) {
			if ( class_exists( 'XL_Spam' ) ) {
				XL_Spam::log_blocked( $form_id, XL_Spam::REASON_HONEYPOT, $email_early, $source_url_early, $ua_early, $ip_early );
			}
			// Silent success — bots must not learn they were blocked.
			wp_send_json_success(
				array(
					'message' => __( 'Thank you for your submission!', 'xtremeleads' ),
					'redirect' => '',
					'spam' => true,
				)
			);
		...
		$form_time = isset( $_POST['xl_form_time'] ) ? absint( $_POST['xl_form_time'] ) : 0;
		if ( $form_time > 0 && ( time() - $form_time ) < 2 ) {
			if ( class_exists( 'XL_Spam' ) ) {
				XL_Spam::log_blocked( $form_id, XL_Spam::REASON_TIMEGATE, $email_early, $source_url_early, $ua_early, $ip_early );
			}
			wp_send_json_success(
				array(
					'message' => __( 'Thank you for your submission!', 'xtremeleads' ),
		...
		if ( ! $form ) {
			wp_send_json_error( array( 'message' => __( 'Form not found.', 'xtremeleads' ) ), 404 );
		}
		...
		// ── reCAPTCHA v3 check ────────────────────────────────────────────────
		if ( class_exists( 'XL_Spam' ) && XL_Spam::is_recaptcha_enabled_for_form( $settings ) ) {
			$recaptcha_token = isset( $_POST['xl_recaptcha_token'] )
				? sanitize_text_field( wp_unslash( $_POST['xl_recaptcha_token'] ) )
				: '';
			$rc_settings = XL_Spam::get_recaptcha_settings();
			$rc_result = XL_Spam::verify_recaptcha( $recaptcha_token, $rc_settings['threshold'], $rc_settings['secret_key'] );

			if ( ! $rc_result['success'] && ! $rc_result['api_failed'] ) {
				XL_Spam::log_blocked( $form_id, XL_Spam::REASON_RECAPTCHA, $email_early, $source_url_early, $ua_early, $ip_early );
				wp_send_json_success(
					array(
						'message' => __( 'Thank you for your submission!', 'xtremeleads' ),
						'redirect' => '',
			...
			if ( $rc_result['api_failed'] ) {
				...
				error_log(
					'XtremeLeads: reCAPTCHA v3 API verification failed for form #' . $form_id.
				...
				if ( class_exists( 'XL_Spam' ) ) {
					XL_Spam::log_blocked(
						$form_id,
						XL_Spam::REASON_RECAPTCHA_API_WARN,
						$email_early,
						$source_url_early,
						$ua_early,
						$ip_early
		...
		$source_url = isset( $_SERVER['HTTP_REFERER'] )
			? esc_url_raw( wp_unslash( $_SERVER['HTTP_REFERER'] ) )
			: '';
		if ( ! $source_url && isset( $_POST['xl_source_url'] ) ) {
			$source_url = esc_url_raw( wp_unslash( $_POST['xl_source_url'] ) );
		}
		...
		foreach ( $fields as $field ) {
			...
			if ( $required ) {
				...
				if ( $is_empty ) {
					...
					$errors[ $fid ] = sprintf( __( '%s is required.', 'xtremeleads' ), $field['label'] ?? $fid );
					continue;
				}
			}
			...
			if ( 'email' === $ftype && '' !== trim( (string) $sanitized ) ) {
				if ( ! is_email( $sanitized ) ) {
					$errors[ $fid ] = __( 'Please enter a valid email address.', 'xtremeleads' );
					continue;
				}
			...
			if ( 'phone' === $ftype && '' !== trim( (string) $sanitized ) ) {
				if ( ! preg_match( '/^[0-9\s\+\-\(\)]+$/', $sanitized ) ) {
					$errors[ $fid ] = __( 'Please enter a valid phone number.', 'xtremeleads' );
					continue;
				}
			}
		...
		if ( ! empty( $settings['consent_enabled'] ) && '1' === (string) $settings['consent_enabled'] ) {
			$consent_given = isset( $_POST['xl_consent'] ) && '1' === sanitize_text_field( wp_unslash( $_POST['xl_consent'] ) );
			if ( ! $consent_given ) {
				$errors['_consent'] = __( 'You must accept the consent checkbox to submit this form.', 'xtremeleads' );
			}
		}

		// ── Blocklist checks ─────────────────────────────────────────
		// Done after field parsing so we have sanitised values.
		if ( class_exists( 'XL_Spam' ) && empty( $errors ) ) {
			...
			if ( '' !== $email_for_blocklist && XL_Spam::is_domain_blocked( $email_for_blocklist ) ) {
				XL_Spam::log_blocked( $form_id, XL_Spam::REASON_BLOCKLIST, $email_for_blocklist, $source_url_early, $ua_early, $ip_early );
			...
			// Keyword blocklist.
			if ( XL_Spam::has_blocked_keyword( $field_values ) ) {
				XL_Spam::log_blocked( $form_id, XL_Spam::REASON_BLOCKLIST, $email_for_blocklist, $source_url_early, $ua_early, $ip_early );
				wp_send_json_success(
					array(
						'message' => __( 'Thank you for your submission!', 'xtremeleads' ),
		...
		if ( ! empty( $errors ) ) {
			wp_send_json_error(
				array(
					'message' => __( 'Please correct the errors below.', 'xtremeleads' ),
					'errors' => $errors,
				),
				422
			);
		...
		$utm_from_url = XL_UTM::extract_from_url( $source_url );

		// Cookie fallback: the front-end sets an xl_utm cookie with UTM data.
		$utm_cookie_json = isset( $_POST['xl_utm_cookie'] )
			? sanitize_text_field( wp_unslash( $_POST['xl_utm_cookie'] ) )
			: '';
		$utm_from_cookie = XL_UTM::extract_from_cookie( $utm_cookie_json );

		// Merge: URL params take precedence; cookie fills in gaps.
		$utm_data = XL_UTM::merge_with_fallback( $utm_from_url, $utm_from_cookie );

		// ── Submit duration (time-to-submit in seconds) ─────────────
		$submit_duration = isset( $_POST['xl_submit_duration'] )
			? absint( $_POST['xl_submit_duration'] )
			: null;
		...
		$submitted_email = XL_Duplicates::extract_email( $field_values, $fields );
		...
		if ( $submitted_email ) {
			...
			// either record is written (see XL_Duplicates class for full docs).
			$lock_acquired = XL_Duplicates::acquire_lock( $submitted_email );

			$original_lead = XL_Duplicates::find_original_by_email( $submitted_email );

			if ( $original_lead ) {
				$behavior = XL_Duplicates::get_behavior();
				$duplicate_result = XL_Duplicates::handle( $original_lead, $field_values, $behavior );
				...
				if ( $duplicate_result['merged'] ) {
					...
					$thank_you = ! empty( $settings['thank_you_message'] )
						...
						: esc_html__( 'Thank you! Your submission has been received.', 'xtremeleads' );

					$redirect_nonce = $redirect_url ? wp_create_nonce( 'xl_form_redirect_' . $form_id ) : '';
		...
		if ( null !== $duplicate_result && ! $duplicate_result['blocked'] && ! $duplicate_result['merged'] ) {
			...
			if ( $orig_id_candidate ) {
				$still_exists = XL_Leads::get_lead( (int) $orig_id_candidate );
				if ( $still_exists ) {
		...
		$consent_given_val = 0;
		if ( ! empty( $settings['consent_enabled'] ) && '1' === (string) $settings['consent_enabled'] ) {
			$consent_given_val = ( isset( $_POST['xl_consent'] ) && '1' === sanitize_text_field( wp_unslash( $_POST['xl_consent'] ) ) ) ? 1 : 0;
		}
		...
		XL_Email::send_new_lead_notification(
			$lead_id,
			$field_values,
			$fields,
			$settings,
		...
		if ( class_exists( 'XL_Webhooks' ) ) {
			$webhook_payload = XL_Webhooks::build_payload(
				$lead_id,
				$field_values,
				$source_url,
				$ip,
			...
			XL_Webhooks::fire_event( XL_Webhooks::EVENT_NEW_LEAD, $lead_id, $webhook_payload, $form_id );
		}
	...
	public function handle_do_form_redirect(): void {
		...
		if ( ! $form_id ) {
			wp_die( esc_html__( 'Invalid form.', 'xtremeleads' ) );
		}

		$nonce = isset( $_POST['xl_redirect_nonce'] )
			? sanitize_text_field( wp_unslash( $_POST['xl_redirect_nonce'] ) )
			: '';

		if ( ! wp_verify_nonce( $nonce, 'xl_form_redirect_' . $form_id ) ) {
			wp_die( esc_html__( 'Security check failed. Please refresh and try again.', 'xtremeleads' ) );
		}
	...
	public function handle_get_lead(): void {
		...
		if ( ! $lead_id ) {
			wp_send_json_error( array( 'message' => __( 'Invalid lead ID.', 'xtremeleads' ) ), 400 );
		}

		$lead = XL_Leads::get_lead( $lead_id );
		if ( ! $lead ) {
			wp_send_json_error( array( 'message' => __( 'Lead not found. It may have been deleted.', 'xtremeleads' ) ), 404 );
		}
		...
		$assignee_name = __( 'Unassigned', 'xtremeleads' );
		...
		// Notes (oldest first).
		$notes_raw = XL_Notes::get_notes_for_lead( $lead_id );
		...
		$activity_raw = XL_Activity::get_activity_for_lead( $lead_id );
		...
		wp_send_json_success(
			array(
				...
				'form_name' => $form ? (string) $form->name : __( '(deleted form)', 'xtremeleads' ),
	...
	public function handle_bulk_action(): void {
		...
		if ( empty( $ids ) ) {
			wp_send_json_error( array( 'message' => __( 'No items selected.', 'xtremeleads' ) ), 422 );
		}
		...
		switch ( $action ) {
			case 'delete':
				...
				if ( class_exists( 'XL_Audit_Log' ) ) {
					foreach ( $ids as $del_id ) {
						XL_Audit_Log::record(
							XL_Audit_Log::ACTION_LEAD_DATA_DELETED,
							(int) $del_id,
				...
				wp_send_json_success(
					array(
						...
						'message' => sprintf( _n( '%d lead deleted.', '%d leads deleted.', $count, 'xtremeleads' ), $count ),
						'count' => $count,
					)
				);
				break;
			...
			case 'mark_contacted':
				...
				wp_send_json_success(
					array(
						...
						'message' => sprintf( _n( '%d lead marked as contacted.', '%d leads marked as contacted.', $count, 'xtremeleads' ), $count ),
						'count' => $count,
					)
				);
				break;
			...
			default:
				wp_send_json_error( array( 'message' => __( 'Unknown action.', 'xtremeleads' ) ), 400 );
		}
	}
	...
	public function handle_add_note(): void {
		...
		$lead = XL_Leads::get_lead( $lead_id );
		if ( ! $lead ) {
			wp_send_json_error( array( 'message' => __( 'Lead not found.', 'xtremeleads' ) ), 404 );
		}
		...
		if ( '' === trim( $content ) ) {
			wp_send_json_error( array( 'message' => __( 'Note content cannot be empty.', 'xtremeleads' ) ), 422 );
		}

		$author_id = get_current_user_id();
		$result = XL_Notes::insert_note( $lead_id, $author_id, $content );
		...
		XL_Activity::log(
			$lead_id,
			$author_id,
			XL_Activity::TYPE_NOTE_ADDED,
			array( 'note_id' => $result )
		);

		// Audit log.
		if ( class_exists( 'XL_Audit_Log' ) ) {
			XL_Audit_Log::record( XL_Audit_Log::ACTION_LEAD_NOTE_ADDED, $lead_id );
		}
		...
		wp_send_json_success(
			array(
				'note' => array(
					...
					'author_name' => $author ? $author->display_name : __( 'Unknown', 'xtremeleads' ),
	...
	public function handle_update_status(): void {
		...
		if ( ! array_key_exists( $new_status, $statuses ) ) {
			wp_send_json_error( array( 'message' => __( 'Invalid status.', 'xtremeleads' ) ), 400 );
		}

		$old_status = XL_Leads::update_status( $lead_id, $new_status );

		if ( false === $old_status ) {
			wp_send_json_error( array( 'message' => __( 'Failed to update status.', 'xtremeleads' ) ), 500 );
		}
		...
		XL_Activity::log(
			$lead_id,
			$user_id,
			XL_Activity::TYPE_STATUS_CHANGE,
			array(
				'from' => $old_status,
				'to' => $new_status,
		...
		if ( class_exists( 'XL_Audit_Log' ) ) {
			XL_Audit_Log::record(
				XL_Audit_Log::ACTION_LEAD_STATUS_CHANGED,
				$lead_id,
				array(
					'from' => $old_status,
					'to' => $new_status,
		...
		if ( class_exists( 'XL_Webhooks' ) ) {
			$lead_obj_sc = XL_Leads::get_lead( $lead_id );
			if ( $lead_obj_sc ) {
				$fv_sc = XL_Leads::decode_field_values( $lead_obj_sc );
				$webhook_payload_sc = XL_Webhooks::build_payload(
					$lead_id,
					$fv_sc,
				...
				XL_Webhooks::fire_event(
					XL_Webhooks::EVENT_STATUS_CHANGE,
					$lead_id,
					$webhook_payload_sc,
	...
	public function handle_search_tags(): void {
		...
		$tags = XL_Tags::search_tags( $query );
		$tags_data = array_map(
			static function ( $t ) {
				return array( 'id' => (int) $t->id, 'name' => (string) $t->name );
			},
	...
	public function handle_apply_tag(): void {
		...
		if ( ! $lead_id || ! $tag_id ) {
			wp_send_json_error( array( 'message' => __( 'Invalid parameters.', 'xtremeleads' ) ), 400 );
		}
		...
		// Verify tag exists.
		$tag = XL_Tags::get_tag( $tag_id );
		if ( ! $tag ) {
			wp_send_json_error( array( 'message' => __( 'Tag not found.', 'xtremeleads' ) ), 404 );
		}

		XL_Tags::apply_tag_to_lead( $lead_id, $tag_id );

		// Log activity.
		XL_Activity::log(
			$lead_id,
			get_current_user_id(),
			XL_Activity::TYPE_TAG_ADDED,
	...
	public function handle_remove_tag(): void {
		...
		XL_Tags::remove_tag_from_lead( $lead_id, $tag_id );
	...
	public function handle_assign_lead(): void {
		...
		if ( $assigned_to > 0 && ! XL_Leads::is_eligible_assignee( $assigned_to ) ) {
			wp_send_json_error(
				array( 'message' => __( 'The selected user is not eligible for lead assignment.', 'xtremeleads' ) ),
				403
			);
		}
		...
		$old_name = $old_user ? $old_user->display_name : __( 'Unassigned', 'xtremeleads' );

		$result = XL_Leads::update_assigned_to( $lead_id, $assigned_to );

		if ( false === $result ) {
			wp_send_json_error( array( 'message' => __( 'Failed to update assignment.', 'xtremeleads' ) ), 500 );
		}

		// New assignee info.
		$new_user = $assigned_to ? get_userdata( $assigned_to ) : null;
		$new_name = $new_user ? $new_user->display_name : __( 'Unassigned', 'xtremeleads' );

		// Log activity.
		XL_Activity::log(
			$lead_id,
			$actor_id,
			XL_Activity::TYPE_ASSIGNMENT,
			array(
		...
		if ( class_exists( 'XL_Audit_Log' ) ) {
			XL_Audit_Log::record(
				XL_Audit_Log::ACTION_LEAD_ASSIGNMENT_CHANGED,
				$lead_id,
				array(
					'from_id' => $old_assignee,
					'from_name' => $old_name,
		...
		if ( $assigned_to && $new_user ) {
			...
			if ( ! $email_sent ) {
				$email_warning = __( 'Assignment saved, but the notification email could not be sent (the user may not have an email address).', 'xtremeleads' );
			}
		}
	...
	private function send_assignment_email( int $lead_id, object $lead, object $new_user, int $actor_id ): bool {
		...
		$actor_name = $actor ? $actor->display_name : __( 'An administrator', 'xtremeleads' );
		...
		$subject = sprintf(
			...
			__( '[XtremeLeads] Lead #%d has been assigned to you', 'xtremeleads' ),
			$lead_id
		);

		$body = '<h2>' . esc_html__( 'New Lead Assignment', 'xtremeleads' ) . '</h2>';
		$body .= '<p>' . sprintf(
			/* translators: 1: lead ID, 2: actor name */
			esc_html__( 'Lead #%1$d has been assigned to you by %2$s.', 'xtremeleads' ),
			$lead_id,
			esc_html( $actor_name )
		) . '</p>';
		$body .= '<p><strong>' . esc_html__( 'Lead:', 'xtremeleads' ) . '</strong> ' . esc_html( $lead_name ) . '</p>';
		$body .= '<p><a href="' . esc_url( $admin_link ) . '">' . esc_html__( 'View Lead in Admin', 'xtremeleads' ) . '</a></p>';
	...
	public function handle_get_eligible_users(): void {
		...
		$users = XL_Leads::get_eligible_assignees();
	...
	public function handle_send_test_email(): void {
		...
		if ( ! wp_verify_nonce( $nonce, 'xl_admin_nonce' ) ) {
			wp_send_json_error( array( 'message' => __( 'Security check failed.', 'xtremeleads' ) ), 403 );
		}

		if ( ! current_user_can( 'manage_options' ) ) {
			wp_send_json_error( array( 'message' => __( 'Permission denied.', 'xtremeleads' ) ), 403 );
		}

		$result = XL_Email::send_test_email();
	...
	public function handle_resend_email(): void {
		...
		if ( ! $log_id ) {
			wp_send_json_error( array( 'message' => __( 'Invalid log ID.', 'xtremeleads' ) ) );
		}

		$result = XL_Email::resend_from_log( $log_id );
	 ...
	 * Auth: Nonce-verified (tied to action 'xl_impression_nonce').
	...
	public function handle_track_impression(): void {
		...
		if ( ! wp_verify_nonce( $nonce, 'xl_impression_nonce' ) ) {
		...
		if ( ! $form_id ) {
			wp_send_json_error( array( 'message' => __( 'Invalid form ID.', 'xtremeleads' ) ), 400 );
		}

		// Validate form_id corresponds to an existing, active form.
		$form = XL_Forms::get_form( $form_id );
		if ( ! $form || 'active' !== $form->status ) {
			wp_send_json_error( array( 'message' => __( 'Form not found or inactive.', 'xtremeleads' ) ), 422 );
		}
	...
	public function handle_dashboard_stats(): void {
		$this->check_analytics_ajax( 'xl_dashboard_stats_nonce' );

		wp_send_json_success(
			array(
				'kpi' => array(
					'all_time' => XL_Analytics::count_leads_all_time(),
					'this_month' => XL_Analytics::count_leads_this_month(),
					'this_week' => XL_Analytics::count_leads_this_week(),
				),
				'funnel' => XL_Analytics::leads_by_status(),
				'top_source_pages' => XL_Analytics::top_source_pages( 10 ),
	...
	public function handle_chart_leads_by_form(): void {
		$this->check_analytics_ajax( 'xl_chart_leads_by_form_nonce' );
		...
		$data = XL_Analytics::leads_by_form( $from_utc, $to_utc );
	...
	public function handle_chart_leads_over_time(): void {
		$this->check_analytics_ajax( 'xl_chart_leads_over_time_nonce' );
		...
		switch ( $range ) {
			...
			case 'custom':
				if ( empty( $date_from ) || empty( $date_to ) ) {
					wp_send_json_error(
						array( 'message' => __( 'Start and end dates are required for a custom range.', 'xtremeleads' ) ),
						400
					);
				}
				...
				if ( ! $start_dt || ! $end_dt ) {
					wp_send_json_error( array( 'message' => __( 'Invalid date format.', 'xtremeleads' ) ), 400 );
				}
				if ( $end_dt < $start_dt ) {
					wp_send_json_error(
						array( 'message' => __( 'End date cannot be before start date.', 'xtremeleads' ) ),
						400
					);
				}
				break;
		...
		$result = XL_Analytics::leads_over_time( $date_from, $date_to );
	...
	public function handle_utm_report(): void {
		$this->check_analytics_ajax( 'xl_utm_report_nonce' );

		$data = XL_Analytics::utm_breakdown();
	...
	public function handle_form_metrics(): void {
		$this->check_analytics_ajax( 'xl_form_metrics_nonce' );
	 ...
	 * Method: POST
	 * Auth: Nonce-verified (tied to action 'xl_duplicate_check_nonce').
	 * Requires manage_options capability.
	...
	public function handle_duplicate_check(): void {
		...
		$this->check_analytics_ajax( 'xl_duplicate_check_nonce' );
		...
		if ( empty( $email ) ) {
			wp_send_json_error( array( 'message' => __( 'Email address is required.', 'xtremeleads' ) ), 400 );
		}
	...
	public function handle_webhook_save(): void {
		...
		$id = XL_Webhooks::save( $raw );
		if ( false === $id ) {
			wp_send_json_error( array( 'message' => __( 'Failed to save webhook.', 'xtremeleads' ) ), 500 );
		}

		$webhook = XL_Webhooks::get( $id );
	...
	public function handle_webhook_delete(): void {
		...
		if ( ! $webhook_id ) {
			wp_send_json_error( array( 'message' => __( 'Invalid webhook ID.', 'xtremeleads' ) ), 400 );
		}

		$result = XL_Webhooks::delete( $webhook_id );
		if ( ! $result ) {
			wp_send_json_error( array( 'message' => __( 'Failed to delete webhook.', 'xtremeleads' ) ), 500 );
		}
	...
	public function handle_webhook_test(): void {
		...
		$result = XL_Webhooks::test_fire( $webhook_id );
	...
	public function handle_webhook_log(): void {
		...
		if ( $webhook_id ) {
			$result = XL_Webhooks::get_log( $webhook_id, $page );
		} else {
			$result = XL_Webhooks::get_all_log( $page );
		}
	...
	public function handle_webhook_get(): void {
		...
		if ( ! $webhook ) {
			...
			wp_send_json_success( array( 'webhooks' => XL_Webhooks::get_all() ) );
			return;
		}
	...
	public function handle_gdpr_erase(): void {
		...
		if ( ! is_email( $email ) ) {
			wp_send_json_error( array( 'message' => __( 'Please enter a valid email address.', 'xtremeleads' ) ), 400 );
		}

		$result = XL_GDPR::erase_by_email( $email );

		// Audit log per deleted lead record.
		if ( class_exists( 'XL_Audit_Log' ) && ! empty( $result['deleted_ids'] ) && is_array( $result['deleted_ids'] ) ) {
		...
		if ( ! $result['found'] ) {
			wp_send_json_error(
				array( 'message' => __( 'No records found for this email address.', 'xtremeleads' ) ),
				404
			);
		}
	...
	public function handle_spam_log_delete(): void {
		...
		if ( ! $entry_id ) {
			wp_send_json_error( array( 'message' => __( 'Invalid entry ID.', 'xtremeleads' ) ), 400 );
		}

		$result = XL_Spam::delete_log_entry( $entry_id );
		if ( ! $result ) {
			wp_send_json_error( array( 'message' => __( 'Failed to delete entry.', 'xtremeleads' ) ), 500 );
		}
