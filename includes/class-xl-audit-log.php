...
class XL_Audit_Log {
	...
	public static function record(
		...
		$user_display = $user ? $user->display_name : __( 'System', 'xtremeleads' );

		$table = $wpdb->prefix . 'xtremeleads_audit_log';
	...
	public static function get_entries( array $args = array() ): array {
		...
		if ( ! empty( $params ) ) {
			...
			$total = (int) $wpdb->get_var( $wpdb->prepare( $total_query, $params ) ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery
		} else {
	...
	public static function get_action_label( string $action_type ): string {
		$labels = array(
			self::ACTION_LEAD_STATUS_CHANGED => __( 'Lead Status Changed', 'xtremeleads' ),
			self::ACTION_LEAD_NOTE_ADDED => __( 'Lead Note Added', 'xtremeleads' ),
			self::ACTION_LEAD_ASSIGNMENT_CHANGED => __( 'Lead Assignment Changed', 'xtremeleads' ),
			self::ACTION_LEAD_DATA_DELETED => __( 'Lead Data Deleted', 'xtremeleads' ),
			self::ACTION_FORM_CREATED => __( 'Form Created', 'xtremeleads' ),
			self::ACTION_FORM_UPDATED => __( 'Form Updated', 'xtremeleads' ),
			self::ACTION_FORM_DELETED => __( 'Form Deleted', 'xtremeleads' ),
			self::ACTION_GLOBAL_SETTINGS_UPDATED => __( 'Global Settings Updated', 'xtremeleads' ),
			self::ACTION_EMAIL_SENT => __( 'Email Sent', 'xtremeleads' ),
			self::ACTION_PLUGIN_DATA_EXPORTED => __( 'Plugin Data Exported', 'xtremeleads' ),
			self::ACTION_PLUGIN_DATA_IMPORTED => __( 'Plugin Data Imported', 'xtremeleads' ),
		);
