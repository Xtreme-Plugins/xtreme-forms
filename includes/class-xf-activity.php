		...
		switch ( $row->action_type ) {
			case self::TYPE_STATUS_CHANGE:
				return sprintf(
					...
					__( '%1$s changed status from <strong>%2$s</strong> to <strong>%3$s</strong>', 'xtreme-forms' ),
			...
			case self::TYPE_ASSIGNMENT:
				return sprintf(
					...
					__( '%1$s assigned lead from <strong>%2$s</strong> to <strong>%3$s</strong>', 'xtreme-forms' ),
					esc_html( $row->user_name ),
					esc_html( $data['from_name'] ?? __( 'Unassigned', 'xtreme-forms' ) ),
					esc_html( $data['to_name'] ?? __( 'Unassigned', 'xtreme-forms' ) )
				);

			case self::TYPE_NOTE_ADDED:
				return sprintf(
					/* translators: %s: user name */
					__( '%s added a note', 'xtreme-forms' ),
					esc_html( $row->user_name )
				);
			...
			case self::TYPE_TAG_ADDED:
				return sprintf(
					...
					__( '%1$s added tag <strong>%2$s</strong>', 'xtreme-forms' ),
					esc_html( $row->user_name ),
			...
			case self::TYPE_TAG_REMOVED:
				return sprintf(
					/* translators: 1: user name, 2: tag name */
					__( '%1$s removed tag <strong>%2$s</strong>', 'xtreme-forms' ),
					esc_html( $row->user_name ),
					esc_html( $data['tag_name'] ?? '' )
				);
			...
			case self::TYPE_LEAD_CREATED:
				return sprintf(
					...
					__( 'Lead created via form <strong>%s</strong>', 'xtreme-forms' ),
