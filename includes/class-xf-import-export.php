...
class XF_Import_Export {
	...
	public static function parse_import( string $json ): array|WP_Error {
		...
		// Schema validation.
		$export_version = $data['xf_export_version'] ?? '';
		if ( '' === $export_version ) {
			return new WP_Error( 'missing_version', __( 'Import file is missing "xf_export_version". This file was not created by Xtreme Forms or is corrupted.', 'xtreme-forms' ) );
		}
	...
	private static function import_forms( array $forms ): array {
		...
		foreach ( $forms as $form_data ) {
			...
			$new_form_id = XF_Forms::create_form( $name, $fields, $settings );
