 ...
 *
 * Export format (v2.4.0+):
 * {
 * "xtremeforms_export_version": "1",
...
/**
 * Class Xtremeforms_Import_Export
 */
class Xtremeforms_Import_Export {
	...
	public static function build_full_export(): array {
		...
		$forms_raw = Xtremeforms_Forms::get_all_forms();
		$forms     = array();
		...
		return array(
			'xtremeforms_export_version' => self::EXPORT_VERSION,
	...
	public static function build_form_export( int $form_id ): array|WP_Error {
		...
		if ( ! $form ) {
			return new WP_Error( 'form_not_found', __( 'Form not found.', 'xtreme-forms' ) );
		}
	...
	private static function get_routing_rules_export(): array {
		global $wpdb;
		$table = $wpdb->prefix . 'xtremeforms_routing_rules';
	...
	public static function stream_json_download( array $data, string $filename ): void {
		...
		if ( false === $json ) {
			wp_die( esc_html__( 'Failed to encode export data.', 'xtreme-forms' ) );
		}
	...
	public static function parse_import( string $json ): array|WP_Error {
		...
		if ( JSON_ERROR_NONE !== json_last_error() ) {
			return new WP_Error(
				...
				sprintf(
					...
					__( 'Invalid JSON: %s', 'xtreme-forms' ),
					json_last_error_msg()
				)
			);
		}
		...
		if ( ! is_array( $data ) ) {
			return new WP_Error( 'invalid_structure', __( 'Import file must contain a JSON object.', 'xtreme-forms' ) );
		}

		// Schema validation. Accept both the new "xtremeforms_export_version"
		// key and the legacy "xf_export_version" key so users can still import
		// older export files generated by plugin versions prior to 2.4.0.
		$export_version = $data['xtremeforms_export_version'] ?? ( $data['xf_export_version'] ?? '' );
		if ( '' === $export_version ) {
			return new WP_Error( 'missing_version', __( 'Import file is missing "xtremeforms_export_version". This file was not created by Xtreme Forms or is corrupted.', 'xtreme-forms' ) );
		}
		...
		if ( version_compare( (string) $export_version, self::MIN_EXPORT_VERSION, '<' ) ) {
			return new WP_Error(
				...
				sprintf(
					...
					__( 'This export file uses format version %1$s which is no longer supported (minimum: %2$s).', 'xtreme-forms' ),
		...
		if ( ! in_array( $type, array( 'full', 'form' ), true ) ) {
			return new WP_Error( 'invalid_type', __( 'Import file has an unrecognized "type" field. Expected "full" or "form".', 'xtreme-forms' ) );
		}

		if ( ! isset( $data['forms'] ) || ! is_array( $data['forms'] ) ) {
			return new WP_Error( 'missing_forms', __( 'Import file is missing the "forms" array.', 'xtreme-forms' ) );
		}
	...
	public static function execute_full_import( array $data ): array {
		...
		if ( ! empty( $data['settings'] ) && is_array( $data['settings'] ) ) {
			...
			update_option( 'xtremeforms_settings', array_merge( $existing, $imported ) );
		}

		// Email template.
		if ( ! empty( $data['email_template'] ) && is_array( $data['email_template'] ) ) {
			$existing_tmpl = get_option( 'xtremeforms_email_template', array() );
			update_option( 'xtremeforms_email_template', array_merge( $existing_tmpl, $data['email_template'] ) );
		}
		...
		if ( class_exists( 'Xtremeforms_Audit_Log' ) ) {
			Xtremeforms_Audit_Log::record(
				Xtremeforms_Audit_Log::ACTION_PLUGIN_DATA_IMPORTED,
				0,
				array(
					'type'                   => 'full',
	...
	private static function import_forms( array $forms ): array {
		...
		foreach ( $forms as $form_data ) {
			...
			$new_form_id = Xtremeforms_Forms::create_form( $name, $fields, $settings );
			...
			if ( class_exists( 'Xtremeforms_Audit_Log' ) ) {
				Xtremeforms_Audit_Log::record(
					Xtremeforms_Audit_Log::ACTION_FORM_CREATED,
					$new_form_id,
					array(
						'source'        => 'import',
						'original_name' => $name,
