...
class Xtremeforms_Ajax {
	...
	public function __construct() {
		...
		add_action( 'wp_ajax_xtremeforms_resend_lead_notification', array( $this, 'handle_resend_lead_notification' ) );

		// Audience Insights: device / browser / OS breakdown.
		add_action( 'wp_ajax_xtremeforms_user_agent_report', array( $this, 'handle_user_agent_report' ) );
	...
	public function handle_form_submit(): void {
		...
		$nonce = isset( $_POST['xtremeforms_nonce'] ) ? sanitize_text_field( wp_unslash( $_POST['xtremeforms_nonce'] ) ) : '';
		...
		$source_url_early = isset( $_SERVER['HTTP_REFERER'] )
			...
			: ( isset( $_POST['xtremeforms_source_url'] ) ? esc_url_raw( wp_unslash( $_POST['xtremeforms_source_url'] ) ) : '' );
		$ip_early         = $this->get_visitor_ip();
		$ip_early         = Xtremeforms_Leads::maybe_anonymize_ip( $ip_early );
		...
		if ( '' !== $honeypot_value ) {
			if ( class_exists( 'Xtremeforms_Spam' ) ) {
				Xtremeforms_Spam::log_blocked( $form_id, Xtremeforms_Spam::REASON_HONEYPOT, $email_early, $source_url_early, $ua_early, $ip_early );
			}
		...
		$form_time = isset( $_POST['xtremeforms_form_time'] ) ? absint( $_POST['xtremeforms_form_time'] ) : 0;
		if ( $form_time > 0 && ( time() - $form_time ) < 2 ) {
			if ( class_exists( 'Xtremeforms_Spam' ) ) {
				Xtremeforms_Spam::log_blocked( $form_id, Xtremeforms_Spam::REASON_TIMEGATE, $email_early, $source_url_early, $ua_early, $ip_early );
			}
			wp_send_json_success(
				array(
					'message'  => __( 'Thank you for your submission!', 'xtreme-forms' ),
		...
		// Load the form.
		$form = Xtremeforms_Forms::get_form( $form_id );
		if ( ! $form ) {
			wp_send_json_error( array( 'message' => __( 'Form not found.', 'xtreme-forms' ) ), 404 );
		}

		$fields   = Xtremeforms_Forms::decode_fields( $form );
		$settings = Xtremeforms_Forms::decode_settings( $form );

		// ── reCAPTCHA v3 check ────────────────────────────────────────────────
		if ( class_exists( 'Xtremeforms_Spam' ) && Xtremeforms_Spam::is_recaptcha_enabled_for_form( $settings ) ) {
			$recaptcha_token = isset( $_POST['xtremeforms_recaptcha_token'] )
				? sanitize_text_field( wp_unslash( $_POST['xtremeforms_recaptcha_token'] ) )
				: '';
			$rc_settings     = Xtremeforms_Spam::get_recaptcha_settings();
			$rc_result       = Xtremeforms_Spam::verify_recaptcha( $recaptcha_token, $rc_settings['threshold'], $rc_settings['secret_key'] );

			if ( ! $rc_result['success'] && ! $rc_result['api_failed'] ) {
				Xtremeforms_Spam::log_blocked( $form_id, Xtremeforms_Spam::REASON_RECAPTCHA, $email_early, $source_url_early, $ua_early, $ip_early );
				wp_send_json_success(
					array(
						'message'  => __( 'Thank you for your submission!', 'xtreme-forms' ),
						'redirect' => '',
			...
			if ( $rc_result['api_failed'] ) {
				...
				if ( class_exists( 'Xtremeforms_Spam' ) ) {
					Xtremeforms_Spam::log_blocked(
						$form_id,
						Xtremeforms_Spam::REASON_RECAPTCHA_API_WARN,
						$email_early,
						$source_url_early,
						$ua_early,
						$ip_early
		...
		if ( class_exists( 'Xtremeforms_Spam' ) ) {
			$turnstile_cfg = Xtremeforms_Spam::get_turnstile_settings();
			if ( $turnstile_cfg['enabled'] ) {
				$turnstile_token = isset( $_POST['xtremeforms_turnstile_token'] )
					? sanitize_text_field( wp_unslash( $_POST['xtremeforms_turnstile_token'] ) )
					: '';
				$ts_result = Xtremeforms_Spam::verify_turnstile( $turnstile_token, $turnstile_cfg['secret_key'] );

				if ( ! $ts_result['success'] && ! $ts_result['api_failed'] ) {
					Xtremeforms_Spam::log_blocked( $form_id, Xtremeforms_Spam::REASON_TURNSTILE, $email_early, $source_url_early, $ua_early, $ip_early );
					wp_send_json_success(
						array(
							'message'  => __( 'Thank you for your submission!', 'xtreme-forms' ),
							'redirect' => '',
		...
		$source_url = isset( $_SERVER['HTTP_REFERER'] )
			? esc_url_raw( wp_unslash( $_SERVER['HTTP_REFERER'] ) )
			: '';
		if ( ! $source_url && isset( $_POST['xtremeforms_source_url'] ) ) {
			$source_url = esc_url_raw( wp_unslash( $_POST['xtremeforms_source_url'] ) );
		}
		...
		if ( ! empty( $settings['consent_enabled'] ) && '1' === (string) $settings['consent_enabled'] ) {
			$consent_given = isset( $_POST['xtremeforms_consent'] ) && '1' === sanitize_text_field( wp_unslash( $_POST['xtremeforms_consent'] ) );
			if ( ! $consent_given ) {
		...
		if ( class_exists( 'Xtremeforms_Spam' ) && empty( $errors ) ) {
			...
			if ( '' !== $email_for_blocklist && Xtremeforms_Spam::is_domain_blocked( $email_for_blocklist ) ) {
				Xtremeforms_Spam::log_blocked( $form_id, Xtremeforms_Spam::REASON_BLOCKLIST, $email_for_blocklist, $source_url_early, $ua_early, $ip_early );
			...
			// Keyword blocklist.
			if ( Xtremeforms_Spam::has_blocked_keyword( $field_values ) ) {
				Xtremeforms_Spam::log_blocked( $form_id, Xtremeforms_Spam::REASON_BLOCKLIST, $email_for_blocklist, $source_url_early, $ua_early, $ip_early );
				wp_send_json_success(
					array(
						'message'  => __( 'Thank you for your submission!', 'xtreme-forms' ),
		...
		$ip = Xtremeforms_Leads::maybe_anonymize_ip( $ip );
		...
		$submitted_email  = Xtremeforms_Duplicates::extract_email( $field_values, $fields );
		...
		if ( $submitted_email ) {
			...
			// either record is written (see Xtremeforms_Duplicates class for full docs).
			$lock_acquired = Xtremeforms_Duplicates::acquire_lock( $submitted_email );

			$original_lead = Xtremeforms_Duplicates::find_original_by_email( $submitted_email );

			if ( $original_lead ) {
				$behavior         = Xtremeforms_Duplicates::get_behavior();
				$duplicate_result = Xtremeforms_Duplicates::handle( $original_lead, $field_values, $behavior );

				if ( $duplicate_result['blocked'] ) {
					// Release lock before returning.
					if ( $lock_acquired ) {
						Xtremeforms_Duplicates::release_lock( $submitted_email );
					}
		...
		if ( null !== $duplicate_result && ! $duplicate_result['blocked'] && ! $duplicate_result['merged'] ) {
			...
			if ( $orig_id_candidate ) {
				$still_exists = Xtremeforms_Leads::get_lead( (int) $orig_id_candidate );
				if ( $still_exists ) {
		...
		$consent_given_val = 0;
		if ( ! empty( $settings['consent_enabled'] ) && '1' === (string) $settings['consent_enabled'] ) {
			$consent_given_val = ( isset( $_POST['xtremeforms_consent'] ) && '1' === sanitize_text_field( wp_unslash( $_POST['xtremeforms_consent'] ) ) ) ? 1 : 0;
		}
		...
		$lead_id = Xtremeforms_Leads::insert_lead(
			array(
				'form_id'                 => $form_id,
		...
		Xtremeforms_Activity::log(
			$lead_id,
			0,
			Xtremeforms_Activity::TYPE_LEAD_CREATED,
		...
		Xtremeforms_Email::send_new_lead_notification(
			$lead_id,
			$field_values,
			$fields,
			$settings,
		...
		if ( class_exists( 'Xtremeforms_Webhooks' ) ) {
			$webhook_payload = Xtremeforms_Webhooks::build_payload(
				$lead_id,
				$field_values,
				$source_url,
				$ip,
			...
			Xtremeforms_Webhooks::fire_event( Xtremeforms_Webhooks::EVENT_NEW_LEAD, $lead_id, $webhook_payload, $form_id );
		}
	...
	public function handle_get_lead(): void {
		// Explicit nonce verification (non-fatal here; helper below handles auth + cap + nonce response).
		...
		// Notes (oldest first).
		$notes_raw  = Xtremeforms_Notes::get_notes_for_lead( $lead_id );
		...
		$activity_raw  = Xtremeforms_Activity::get_activity_for_lead( $lead_id );
	...
	public function handle_bulk_action(): void {
		...
		switch ( $action ) {
				...
				if ( class_exists( 'Xtremeforms_Audit_Log' ) ) {
					foreach ( $ids as $del_id ) {
						Xtremeforms_Audit_Log::record(
							Xtremeforms_Audit_Log::ACTION_LEAD_DATA_DELETED,
							(int) $del_id,
							array( 'method' => 'bulk_delete' )
						);
					}
				}
				$count = Xtremeforms_Leads::bulk_delete( $ids );
				wp_send_json_success(
					array(
			...
			case 'mark_contacted':
				$count = Xtremeforms_Leads::bulk_update_status( $ids, Xtremeforms_Leads::STATUS_CONTACTED );
				wp_send_json_success(
					array(
	...
	public function handle_add_note(): void {
		...
		$result    = Xtremeforms_Notes::insert_note( $lead_id, $author_id, $content );
		...
		Xtremeforms_Activity::log(
			$lead_id,
			$author_id,
			Xtremeforms_Activity::TYPE_NOTE_ADDED,
			array( 'note_id' => $result )
		);

		// Audit log.
		if ( class_exists( 'Xtremeforms_Audit_Log' ) ) {
			Xtremeforms_Audit_Log::record( Xtremeforms_Audit_Log::ACTION_LEAD_NOTE_ADDED, $lead_id );
		}
	...
	public function handle_update_status(): void {
		...
		$old_status = Xtremeforms_Leads::update_status( $lead_id, $new_status );
		...
		Xtremeforms_Activity::log(
			$lead_id,
			$user_id,
			Xtremeforms_Activity::TYPE_STATUS_CHANGE,
			array(
				'from'       => $old_status,
				'to'         => $new_status,
		...
		if ( class_exists( 'Xtremeforms_Audit_Log' ) ) {
			Xtremeforms_Audit_Log::record(
				Xtremeforms_Audit_Log::ACTION_LEAD_STATUS_CHANGED,
				$lead_id,
				array(
					'from' => $old_status,
					'to'   => $new_status,
		...
		if ( class_exists( 'Xtremeforms_Webhooks' ) ) {
			$lead_obj_sc = Xtremeforms_Leads::get_lead( $lead_id );
			if ( $lead_obj_sc ) {
				$fv_sc                            = Xtremeforms_Leads::decode_field_values( $lead_obj_sc );
				$webhook_payload_sc               = Xtremeforms_Webhooks::build_payload(
					$lead_id,
					$fv_sc,
				...
				Xtremeforms_Webhooks::fire_event(
					Xtremeforms_Webhooks::EVENT_STATUS_CHANGE,
					$lead_id,
					$webhook_payload_sc,
	...
	public function handle_create_tag(): void {
		...
		$result = Xtremeforms_Tags::create_tag( $name );

		if ( is_wp_error( $result ) ) {
			wp_send_json_error( array( 'message' => $result->get_error_message() ), 422 );
		}

		$tag = Xtremeforms_Tags::get_tag( $result );
		wp_send_json_success(
			array(
				'tag' => array(
					'id'   => (int) $tag->id,
	...
	public function handle_search_tags(): void {
		...
		$tags      = Xtremeforms_Tags::search_tags( $query );
		$tags_data = array_map(
			static function ( $t ) {
				return array(
					'id'   => (int) $t->id,
	...
	public function handle_apply_tag(): void {
		...
		// Verify tag exists.
		$tag = Xtremeforms_Tags::get_tag( $tag_id );
		if ( ! $tag ) {
			wp_send_json_error( array( 'message' => __( 'Tag not found.', 'xtreme-forms' ) ), 404 );
		}

		Xtremeforms_Tags::apply_tag_to_lead( $lead_id, $tag_id );

		// Log activity.
		Xtremeforms_Activity::log(
			$lead_id,
			get_current_user_id(),
			Xtremeforms_Activity::TYPE_TAG_ADDED,
			array(
				'tag_id'   => $tag_id,
				'tag_name' => $tag->name,
			)
	...
	public function handle_remove_tag(): void {
		...
		Xtremeforms_Tags::remove_tag_from_lead( $lead_id, $tag_id );
		...
		if ( $tag ) {
			Xtremeforms_Activity::log(
				$lead_id,
				get_current_user_id(),
				Xtremeforms_Activity::TYPE_TAG_REMOVED,
				array(
					'tag_id'   => $tag_id,
					'tag_name' => $tag->name,
				)
	...
	public function handle_assign_lead(): void {
		...
		if ( $assigned_to > 0 && ! Xtremeforms_Leads::is_eligible_assignee( $assigned_to ) ) {
			wp_send_json_error(
		...
		$result = Xtremeforms_Leads::update_assigned_to( $lead_id, $assigned_to );
		...
		Xtremeforms_Activity::log(
			$lead_id,
			$actor_id,
			Xtremeforms_Activity::TYPE_ASSIGNMENT,
			array(
		...
		if ( class_exists( 'Xtremeforms_Audit_Log' ) ) {
			Xtremeforms_Audit_Log::record(
				Xtremeforms_Audit_Log::ACTION_LEAD_ASSIGNMENT_CHANGED,
				$lead_id,
				array(
					'from_id'   => $old_assignee,
					'from_name' => $old_name,
	...
	public function handle_get_eligible_users(): void {
		...
		$users = Xtremeforms_Leads::get_eligible_assignees();
	...
	public function handle_send_test_email(): void {
		...
		$result = Xtremeforms_Email::send_test_email();
	...
	public function handle_resend_email(): void {
		...
		$result = Xtremeforms_Email::resend_from_log( $log_id );
	...
	public function handle_user_agent_report(): void {
		// Explicit nonce verification (non-fatal here; helper below handles auth + cap + nonce response).
		check_ajax_referer( 'xtremeforms_user_agent_report_nonce', 'nonce', false );
		$this->require_admin_auth( 'xtremeforms_user_agent_report_nonce' );
		...
		$data = Xtremeforms_Analytics::user_agent_breakdown( $date_from, $date_to );
	...
	public function handle_resend_lead_notification(): void {
		...
		$result = Xtremeforms_Email::resend_lead_notification( $lead_id, $recipient );
	...
	public function handle_dashboard_stats(): void {
		// Explicit nonce verification (non-fatal here; helper below handles auth + cap + nonce response).
		check_ajax_referer( 'xtremeforms_dashboard_stats_nonce', 'nonce', false );
		$this->check_analytics_ajax( 'xtremeforms_dashboard_stats_nonce' );

		wp_send_json_success(
			array(
				'kpi'              => array(
					'all_time'   => Xtremeforms_Analytics::count_leads_all_time(),
					'this_month' => Xtremeforms_Analytics::count_leads_this_month(),
					'this_week'  => Xtremeforms_Analytics::count_leads_this_week(),
				),
				'funnel'           => Xtremeforms_Analytics::leads_by_status(),
				'top_source_pages' => Xtremeforms_Analytics::top_source_pages( 10 ),
	...
	public function handle_chart_leads_by_form(): void {
		// Explicit nonce verification (non-fatal here; helper below handles auth + cap + nonce response).
		check_ajax_referer( 'xtremeforms_chart_leads_by_form_nonce', 'nonce', false );
		...
		$data = Xtremeforms_Analytics::leads_by_form( $from_utc, $to_utc );
	...
	public function handle_chart_leads_over_time(): void {
		// Explicit nonce verification (non-fatal here; helper below handles auth + cap + nonce response).
		check_ajax_referer( 'xtremeforms_chart_leads_over_time_nonce', 'nonce', false );
		...
		$result = Xtremeforms_Analytics::leads_over_time( $date_from, $date_to );
	...
	public function handle_utm_report(): void {
		// Explicit nonce verification (non-fatal here; helper below handles auth + cap + nonce response).
		check_ajax_referer( 'xtremeforms_utm_report_nonce', 'nonce', false );
		$this->check_analytics_ajax( 'xtremeforms_utm_report_nonce' );

		$data = Xtremeforms_Analytics::utm_breakdown();
	...
	public function handle_form_metrics(): void {
		// Explicit nonce verification (non-fatal here; helper below handles auth + cap + nonce response).
		check_ajax_referer( 'xtremeforms_form_metrics_nonce', 'nonce', false );
	...
	public function handle_duplicate_check(): void {
		...
		// Explicit nonce verification (non-fatal here; helper below handles auth + cap + nonce response).
		check_ajax_referer( 'xtremeforms_duplicate_check_nonce', 'nonce', false );
		...
		// Case-insensitive lookup via LOWER() comparison — same strategy used in Xtremeforms_Leads duplicate detection.
	...
	public function handle_webhook_save(): void {
		// Explicit nonce verification (non-fatal here; helper below handles auth + cap + nonce response).
		check_ajax_referer( 'xtremeforms_webhook_nonce', 'nonce', false );
	...
	public function handle_webhook_delete(): void {
		...
		$result = Xtremeforms_Webhooks::delete( $webhook_id );
		if ( ! $result ) {
	...
	public function handle_webhook_test(): void {
		...
		$result = Xtremeforms_Webhooks::test_fire( $webhook_id );
	...
	public function handle_webhook_log(): void {
		...
		if ( $webhook_id ) {
			$result = Xtremeforms_Webhooks::get_log( $webhook_id, $page );
		} else {
			$result = Xtremeforms_Webhooks::get_all_log( $page );
		}
	...
	public function handle_webhook_get(): void {
		...
		$webhook_id = isset( $_POST['webhook_id'] ) ? absint( $_POST['webhook_id'] ) : 0;
		$webhook    = $webhook_id ? Xtremeforms_Webhooks::get( $webhook_id ) : null;

		if ( ! $webhook ) {
			// Return all webhooks if no specific ID.
			wp_send_json_success( array( 'webhooks' => Xtremeforms_Webhooks::get_all() ) );
			return;
		}
	...
	public function handle_gdpr_erase(): void {
		// Explicit nonce verification (non-fatal here; helper below handles auth + cap + nonce response).
		check_ajax_referer( 'xtremeforms_gdpr_nonce', 'nonce', false );
		...
		$result = Xtremeforms_GDPR::erase_by_email( $email );

		// Audit log per deleted lead record.
		if ( class_exists( 'Xtremeforms_Audit_Log' ) && ! empty( $result['deleted_ids'] ) && is_array( $result['deleted_ids'] ) ) {
	...
	public function handle_spam_log_get(): void {
		// Explicit nonce verification (non-fatal here; helper below handles auth + cap + nonce response).
		check_ajax_referer( 'xtremeforms_spam_log_nonce', 'nonce', false );
		...
		$result = Xtremeforms_Spam::get_log( $args );
		wp_send_json_success( $result );
	}
	...
	public function handle_spam_log_delete(): void {
		...
		$result = Xtremeforms_Spam::delete_log_entry( $entry_id );
		if ( ! $result ) {
