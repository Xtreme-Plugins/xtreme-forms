...
<?php
/*
 * Form-builder JS bootstrap data + i18n strings.
 *
 * The dedicated JS file (admin/js/xf-form-settings.js) plus the form-builder
 * framework (admin/js/xf-builder.js) are enqueued via the shared admin enqueue
 * function. Per-render data and translatable strings are attached here using
 * wp_add_inline_script() / wp_localize_script() so the WordPress.org Plugin
 * Check sees no inline <script> tags in the rendered HTML.
 */
wp_add_inline_script(
	'xf-builder',
	'var xfBuilderData = ' . wp_json_encode(
		array( 'fields' => $fields ),
...
wp_localize_script(
	...
	'xfFormSettingsI18n',
	array(
		'invalidReplyTo' => __( 'Please enter a valid reply-to email address.', 'xtreme-forms' ),
	)
);
