/* global xtremeFormsRoutingRulesData, xtremeFormsRoutingRulesI18n */
(function () {
	'use strict';

	var data = window.xtremeFormsRoutingRulesData || {};
	var i18n = window.xtremeFormsRoutingRulesI18n || {};

	function t( key, fallback ) {
		return ( i18n && i18n[ key ] ) || fallback;
	}

	let ruleCounter = parseInt( data.ruleCount, 10 ) || 0;
	...
	tbody.addEventListener('click', function (e) {
		if (e.target.classList.contains('xf-remove-rule')) {
			...
			if (tbody.querySelectorAll('.xf-rule-row').length === 0) {
				...
				row.innerHTML = '<td colspan="8" style="text-align:center;color:#6C757D;padding:24px;">' + t( 'noRulesYet', 'No routing rules yet. Click "Add Rule" to create one.' ) + '</td>';
