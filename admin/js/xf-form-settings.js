/* global xtremeFormsFormSettingsI18n */
(function () {
	'use strict';

	function i18n( key, fallback ) {
		return ( window.xtremeFormsFormSettingsI18n && xtremeFormsFormSettingsI18n[ key ] ) || fallback;
	}
	...
	function validateReplyTo() {
		...
		var emailRe = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
		if (!emailRe.test(val)) {
			replyToErr.textContent = i18n( 'invalidReplyTo', 'Please enter a valid reply-to email address.' );
